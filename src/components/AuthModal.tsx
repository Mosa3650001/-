  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/invalid-credential" || err.code === "auth/wrong-password") {
        ...
        msg = "تسجيل الدخول بالبريد الإلكتروني غير مفعّل في Firebase Console (Authentication > Sign-in method > Email/Password).";
      } else if (err.code === "auth/unauthorized-domain") {
        msg = "النطاق الحالي غير مصرح به في Firebase Console (Authentication > Settings > Authorized domains).";
      } else if (err.message) {
        msg = `خطأ (${err.code || "auth/error"}): ${err.message}`;
      }
      setErrorMessage(msg);
    } finally {
      setIsLoading(false);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        setErrorMessage("تم إغلاق نافذة تسجيل الدخول قبل إتمام العملية.");
      } else if (err.code === "auth/operation-not-allowed") {
        setErrorMessage("تسجيل الدخول عبر Google غير مفعّل في Firebase Console (Authentication > Sign-in method > Google).");
      } else if (err.code === "auth/unauthorized-domain") {
        setErrorMessage("النطاق غير مصرح به في Firebase Console (Authentication > Settings > Authorized domains).");
      } else {
        setErrorMessage(`تعذر الدخول عبر Google (${err.code || "خطأ"}). ${err.message || ""}`);
      }
    } finally {
      setIsLoading(false);
    }
    ...
    try {
      ...
      if (matched) {
        ...
        const newMember = createTeamMember({
          ...
          email: user.email || `facebook_${user.uid.substring(0, 6)}@meta.user`,
          role: "editor",
          roleLabel: "مساعد محتوى ومحرر",
          avatar: user.photoURL || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=80",
          assignedBrandIds: ["all"],
      ...
      addToast({
        type: "success",
        title: "تم تسجيل الدخول عبر Facebook بنجاح! 💙",
        description: `أهلاً بك ${user.displayName || "عزيزنا المستخدم"}`,
      });
      onClose();
    } catch (err: any) {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setErrorMessage("هذا البريد مسجل مسبقاً بطريقة دخول أخرى (مثل Google أو البريد). يرجى تسجيل الدخول بها.");
      } else if (err.code === "auth/configuration-not-found" || err.code === "auth/operation-not-allowed") {
        setErrorMessage("يرجى التأكد من تفعيل مزوّد Facebook وإدخال App ID في لوحة تحكم Firebase Console.");
      } else {
        setErrorMessage("تعذر إتمام الدخول عبر Facebook، يرجى المحاولة لاحقاً.");
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#0f172a] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden p-6 md:p-8 space-y-6 text-right relative">
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2.5 rounded-xl border border-blue-200 dark:border-blue-900/60 bg-[#1877F2]/5 dark:bg-[#1877F2]/10 hover:bg-[#1877F2]/10 dark:hover:bg-[#1877F2]/20 text-[#1877F2] dark:text-[#4593f7] text-xs font-bold transition flex items-center justify-center gap-2"
          >
